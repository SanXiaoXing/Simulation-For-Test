...
| mode | MFD 显示模式：`air_to_air`/`air_to_ground`/`air_to_sea` | enum(string) | 变长 |
...
| page | 当前显示页签：`overview`/`air`/`ground`/`sea`/`external` | enum(string) | 变长 |
| ui_command | 交互指令：`start`/`stop`/`toggle_lock`/`next_waypoint` | enum(string) | 变长 |
...
| 输出名称 | 功能说明 | 类型 | 字节数 |
| --- | --- | --- | --- |
| mfd_frame | 单帧 MFD 数据（按 update_hz 周期发送；默认 JSON-over-UDP/TCP） | bytes（UTF-8 JSON） | 变长 |
| ts | 时间戳（Unix 秒，浮点） | float64 | 8（JSON 变长） |
| mode | 当前 MFD 模式（同输入 mode） | string(UTF-8) | 变长 |
| page | 当前显示页（同输入 page） | string(UTF-8) | 变长 |
...
| flight.waypoint_name | 航路点名称（示例：`WP1`） | string(UTF-8) | 变长 |
| flight.waypoint_distance_m | 航路点距离（m） | float64 | 8（JSON 变长） |
...
| weapon.ammo_missile | 导弹弹药量 | int | 变长 |
| weapon.ammo_gun | 航炮弹药量 | int | 变长 |
...
| tactical.is_friend | IFF 友方标识（true 友方/false 敌方） | bool | 1（JSON 变长） |
| tactical.radar_tracks | 雷达航迹数量 | int | 变长 |

注：当启用 ICD 字段筛选（`icd_schema.fields[].name`）时，输出 JSON 可能变为“扁平化键值对”（key 为点分路径，value 为对应字段值）。
