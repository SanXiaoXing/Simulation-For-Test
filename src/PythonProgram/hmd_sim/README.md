...
| mode | HMD 显示模式：`air_to_air`/`air_to_ground`/`air_to_sea` | enum(string) | 变长 |
...
| 输出名称 | 功能说明 | 类型 | 字节数 |
| --- | --- | --- | --- |
| hmd_frame | 单帧 HMD 数据（按 update_hz 周期发送） | bytes（UTF-8 JSON） | 变长 |
| ts | 时间戳（Unix 秒，浮点） | float64 | 8（JSON 变长） |
| mode | 当前 HMD 模式（同输入 mode） | string(UTF-8) | 变长 |
...
| flight.head_yaw_deg | 头部偏航（deg） | float64 | 8（JSON 变长） |
| flight.head_pitch_deg | 头部俯仰（deg） | float64 | 8（JSON 变长） |
| flight.head_roll_deg | 头部滚转（deg） | float64 | 8（JSON 变长） |
...
| weapon.off_boresight_deg | 离轴角（deg） | float64 | 8（JSON 变长） |
| weapon.rmax_m | Rmax（m） | float64 | 8（JSON 变长） |
| weapon.rne_m | Rne（m） | float64 | 8（JSON 变长） |
...
| tactical.is_friend | 敌我属性（true=友方） | bool | 1（JSON 变长） |
